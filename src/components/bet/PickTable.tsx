...
import { AgreementBadge, BetSourceBadge, RatingBadge, RealismFlagBadges, RiskBadge, ScorePill, fmtAmerican, fmtPct } from "./ui";
...
export function PickTable({ rows }: { rows: BetSelection[] }) {
  ...
  const columns: Column<BetSelection>[] = [
      ...
      render: (r) => (
        <div className="min-w-[160px]">
          <div className="flex items-center gap-1.5">
            ...
            {r.explanation ? <span className="cursor-help text-wc-muted/60" title={r.explanation}>ⓘ</span> : null}
          </div>
          {r.realismFlags?.length ? <div className="mt-1"><RealismFlagBadges flags={r.realismFlags} /></div> : null}
        </div>
      ),
    },
    ...
    { key: "score", label: "Score", align: "right", render: (r) => (r.finalValueScore == null ? "—" : <ScorePill score={r.finalValueScore} />) },
    { key: "conf", label: "Conf.", align: "right", render: (r) => r.confidenceScore },
    { key: "agree", label: "Modelos", render: (r) => <AgreementBadge score={r.modelAgreement} label={r.modelAgreementLabel} /> },
